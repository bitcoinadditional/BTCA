...
if [ $# -ne 6 ]; then
    ...
    echo "Usage: $0 BTCA __BTCa__ __bitcoinadditional__ develop 3.0.0.0 Linux"
    exit 1
fi
...
docker build \
	...
	-f ./contrib/docker/Dockerfile.btca-${ARCHITECTURE2}-wallet \
...
if [ "$TICKER" == "BTCA" ]; then
	...
	response=$(curl -s https://api.github.com/repos/bitcoinadditional/${TICKER}/releases/latest)
